...
float4 main(VertexToPixel input) : SV_TARGET
{
    ...
    for (int i = 0; i < NUM_LIGHTS; i++)
    {
        ...
        {
            case 0: //directional
                finalPixelColor += DirectionalLight(lights[i], surfaceColor, input.normal, cameraPosition, input.worldPosition, roughness);
                break;
            case 1: //point
                finalPixelColor += PointLight(lights[i], surfaceColor, input.normal, cameraPosition, input.worldPosition, roughness);
                break;
            case 2: //spot
                break;
        }
